...
(defun insert-into-value-node (neighbor-node key value hash shift test-function)
  (if (value-node-match-p neighbor-node key test-function)
      (values (%make-value-node :hash hash
                                :key key
                                :value value)
              nil)
  ...
  (let* ((index (extract-hash-part-for-shift shift hash)))
    (if (hash-node-contains-p hash-node index)
        ...
        (let* ((existing-child (hash-node-ref hash-node index)))
          (multiple-value-bind (inserted-child added-p) (node-insert existing-child
                                                                     key
                                                                     value
                                                                     hash
                                                                     (1+ shift)
                                                                     test-function)
            (values (hash-node-replace-at hash-node index inserted-child) added-p)))

        ;; If the hash node doesn't have a child there yet, insert one.
        (values (add-to-hash-node hash-node
...
(declaim (ftype (function (node t t fixnum shift test-function) (values node boolean &optional))
                node-insert))
      ...
      (if (null body)
          (%make-dict :size 1
                      ...
                      :body (%make-value-node :hash hash
                                              :key key
                                              :value value))
          (multiple-value-bind (new-body addedp) (node-insert body key value hash 0 test-function)
            (%make-dict :size (+ size (if addedp 1 0))
                        :test-function test-function
                        :hash-function hash-function
                        :body new-body))))))
