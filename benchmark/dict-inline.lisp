(uiop:define-package :immutable/benchmark/dict-inline
  (:use :cl :iterate :immutable/benchmark/utils)
  (:import-from :alexandria
                #:symbolicate)
  (:local-nicknames (#:hash :immutable/hash)
                    (#:dict :immutable/dict-inline))
  (:export #:immutable-dict-inline-suite))
(in-package :immutable/benchmark/dict-inline)

(declaim (optimize (speed 3) (safety 1) (debug 1) (space 1) (compilation-speed 0)))

(def-suite immutable-dict-inline-suite
    (asdf:system-relative-pathname "immutable" "benchmark-data/dict-inline/"))
...
(defmacro define-experiment-for-test-functions (base-name (test-function-binding total-size-binding) &body body)
  (cons 'progn
        (iter (for test-function in '(hash:== equal))
          (collect
              `(define-growing-experiment ,(symbolicate base-name '- test-function) (:suite immutable-dict-inline-suite)
                   (,total-size-binding)
